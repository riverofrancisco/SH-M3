...
/*   async.each(['poem-two/stanza-01.txt', 'poem-two/stanza-02.txt'],
  ...
  // AsyncAwait version
    let values = await Promise.all([promisifiedReadFile('poem-two/stanza-01.txt'), promisifiedReadFile('poem-two/stanza-02.txt')])
    blue(values[0])
    blue(values[1])
    console.log('done')
}
 ...
 */
  // AsyncAwait version
  let promisesARR = filenames.map(name => promisifiedReadFile(name))

  let values = await Promise.all(promisesARR)
  values.forEach(value => blue(value));
  console.log('done')
}
  ...
  // AsyncAwait version
    async function ReadAsync (file) {
      ...
      while(count < filenames.length){
        await ReadAsync(filenames[count]);
        count ++;
      }
    console.log('done');
...
/*   async.eachSeries(filenames,
  ...
  // AsyncAwait version
  /* try{
    Promise.reduce(filenames, (initial, curr) => promisifiedReadFile(curr).then(stanza => blue(stanza)), 0)
  } catch {
    err => magenta(err)
  }
